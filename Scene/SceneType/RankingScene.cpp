...
void RankingScene::ResetRankingFile()
{
    std::ofstream file("ranking.txt", std::ios::trunc);
    for (int i = 0; i < 5; ++i)
        file << (1000.0f * (5 - i)) << ",2025-01-0" << (i + 1) << "\n";
    file.close();
    LoadRankingFromFile();
}
...
eSceneType RankingScene::Update(float delta)
{
    ...
    if (confirm_reset)
    {
        if (input->GetKeyDown(KEY_INPUT_Z) || input->GetButtonDown(0))
        {
            ResetRankingFile();
            confirm_reset = false;
        }
...
void RankingScene::Draw()
{
    ...
    //if (confirm_reset)
    //{
    //    DrawNeonText(cx - 240, 600, "Reset ranking? [Z]/[A]=Yes  [X]/[B]=No",
    //        GetColor(255, 100, 100), GetColor(255, 0, 0), 180);
    //}
    //else
    //{
    //    DrawNeonText(cx - 220, 600, "Press [Z]/[A] to return  [X]/[B] to reset",
